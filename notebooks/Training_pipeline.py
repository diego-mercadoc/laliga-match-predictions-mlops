...
# Initialize DagsHub
dagshub.init(repo_owner="JuanPab2009", repo_name="ProyectoFinalCD", mlflow=True)
...
def validate_dataset(df: pd.DataFrame, df_goals: pd.DataFrame, df_yellow_cards: pd.DataFrame, is_prediction: bool = False) -> bool:
    ...
    Args:
        ...
        is_prediction: Whether we're validating for prediction mode
    ...
    for _, row in df.iterrows():
        try:
            # Get yellow cards count for home and away teams
            ...
            if not is_prediction:
                MatchStats(
                    ...
                    referee=row['Referee'],
                    home_fouls_received=row.get('Fls', 0),  # Use get() with default value
                    away_fouls_received=row.get('Fls_opp', 0)  # Use get() with default value
                )
            else:
                MatchStats(
                    match_id=row['Match_ID'],
                    ...
                    away_yellow_cards=away_yellow_cards,
                    referee=row['Referee'],
                    home_fouls_received=0,  # Default values for prediction mode
                    away_fouls_received=0  # Default values for prediction mode
                )
...
def actualizar_dataset(file_path,jornada_actual) -> pd.DataFrame:
    ...
    ### Estadisticas de Juego (Corners)
    df_corners = tables[6]  # Adjust index based on actual table position
    df_corners.columns = df_corners.columns.droplevel(level=0)
    df_corners = df_corners[['Equipo', 'CaF', 'CaC']]  # Corner stats
    df_corners = df_corners.rename(columns={'CaF': 'Corners_Favor', 'CaC': 'Corners_Contra'})
    
    ### Estadisticas de Disciplina (Cards)
    df_cards = tables[14]  # Adjust index based on actual table position
    df_cards.columns = df_cards.columns.droplevel(level=0)
    df_cards = df_cards[['Equipo', 'TA', 'TR']]  # Yellow and Red cards
    df_cards = df_cards.rename(columns={'TA': 'Amarillas', 'TR': 'Rojas'})
    ...
    df_final = pd.merge(df_final, df_corners, left_on='Equipo', right_on='Equipo', how='left')
    df_final = pd.merge(df_final, df_cards, left_on='Equipo', right_on='Equipo', how='left')
    ...
    columns_to_rename = ['Edad', 'Pos.', 'Ass', 'TPint', 'PrgC', 'PrgP', '% de TT',
                         ...
                         'Máximo Goleador del Equipo', 'Corners_Favor', 'Corners_Contra',
                         'Amarillas', 'Rojas']
...
def preparar_datos_prediccion(jornada_actual: int) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        try:
            ...
            # Find the match report URL using XPath
            match_date = row['Fecha'].strftime('%Y-%m-%d')
            match_xpath = f"//tr[.//td[contains(text(), '{row['Local']}')] and .//td[contains(text(), '{row['Visitante']}')] and .//td[contains(text(), '{match_date}')]]//a[contains(@href, '/Informe/')]/@href"
            match_urls = tree.xpath(match_xpath)
            
            if match_urls:
                match_report_url = "https://fbref.com" + match_urls[0]
    ...
    # Renombramos las columnas
    df = df[["Fecha", "Día", "Sedes", "Visitante", "Local", "Corners_Home", "Corners_Away", "Referee", "Fls", "Fls_opp", "FR", "FR_opp"]]
    df = df.rename(columns={"Local": "Anfitrion", "Visitante": "Adversario"})
    ...
    df_ataque = tables[2]
    logger.info(f"Attack stats columns: {df_ataque.columns.tolist()}")
    # Get the actual column names from the table
    df_ataque.columns = df_ataque.columns.droplevel(level=0)  # Drop the first level if it's a multi-index
    ...
    column_mapping = {
        ...
        'Posición': 'Pos.',
        'Asistencias': 'Ass',
        'Toques en el área penal': 'TPint',
        'Progresión de conducción': 'PrgC',
        'Pases progresivos': 'PrgP'
    }
    
    # Rename columns to match expected names
    df_ataque = df_ataque.rename(columns=column_mapping)
    df_ataque = df_ataque[['Equipo', 'Edad', 'Pos.', 'Ass', 'TPint', 'PrgC', 'PrgP']]

    ### Estadisticas de disparos
    df_disparos = tables[8]
    df_disparos.columns = df_disparos.columns.droplevel(level=0)
    df_disparos = df_disparos[['Equipo', '% de TT', 'Dist']]
    ...
    df_defensa.columns = df_defensa.columns.droplevel(level=0)
    logger.info(f"Defense stats columns: {df_defensa.columns.tolist()}")
    
    # Map old column names to new ones
    defense_column_mapping = {
        'Equipo': 'Equipo',
        'Entradas ganadas': 'TklG',
        'Intercepciones': 'Int',
        'Errores': 'Err'
    }
    ...
    # Merge all stats for home team
    df = df.merge(df_basic, left_on='Anfitrion', right_on='Equipo', how='left')
    ...
    df = df.rename(columns={
        ...
        'xG': 'xG(tm)',
        'xGA': 'xGA(tm)',
        'Últimos 5': 'Últimos 5(tm)',
        'Máximo Goleador del Equipo': 'Máximo Goleador del Equipo(tm)'
    })

    df = df.merge(df_ataque, left_on='Anfitrion', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        'Edad': 'Edad(tm)',
        'Pos.': 'Pos.(tm)',
    ...
    df = df.merge(df_disparos, left_on='Anfitrion', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        '% de TT': '% de TT(tm)',
        'Dist': 'Dist(tm)'
    ...
    df = df.merge(df_pases, left_on='Anfitrion', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        '% Cmp': '% Cmp(tm)',
        'Dist. tot.': 'Dist. tot.(tm)'
    })

    df = df.merge(df_defensa, left_on='Anfitrion', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        'TklG': 'TklG(tm)',
        'Int': 'Int(tm)',
    ...
    # Merge all stats for away team
    df = df.merge(df_basic, left_on='Adversario', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        'RL': 'RL(opp)',
        'PG': 'PG(opp)',
        ...
        'xG': 'xG(opp)',
        'xGA': 'xGA(opp)',
        'Últimos 5': 'Últimos 5(opp)',
        'Máximo Goleador del Equipo': 'Máximo Goleador del Equipo(opp)'
    })

    df = df.merge(df_ataque, left_on='Adversario', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        'Edad': 'Edad(opp)',
        'Pos.': 'Pos.(opp)',
    ...
    df = df.merge(df_disparos, left_on='Adversario', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        '% de TT': '% de TT(opp)',
        'Dist': 'Dist(opp)'
    ...
    df = df.merge(df_pases, left_on='Adversario', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        '% Cmp': '% Cmp(opp)',
        'Dist. tot.': 'Dist. tot.(opp)'
    })

    df = df.merge(df_defensa, left_on='Adversario', right_on='Equipo', how='left')
    df = df.drop('Equipo', axis=1)
    df = df.rename(columns={
        'TklG': 'TklG(opp)',
        'Int': 'Int(opp)',
...
def cargar_procesar_dataset(df):
    ...
    # Define feature sets for each prediction target
    feature_sets = {
        'match_outcome': [
            'Día', 'Sedes',
            # Team features
            ...
            'GC(tm)', 'xG(tm)', 'xGA(tm)', 'Últimos 5(tm)', 'Máximo Goleador del Equipo(tm)',
            # Opponent features
            'Edad(opp)', 'Pos.(opp)', 'Ass(opp)', 'TPint(opp)', 'PrgC(opp)', 'PrgP(opp)',
            '% de TT(opp)', 'Dist(opp)', '% Cmp(opp)', 'Dist. tot.(opp)', 'TklG(opp)',
            'Int(opp)', 'Err(opp)', 'RL(opp)', 'PG(opp)', 'PE(opp)', 'PP(opp)', 'GF(opp)',
            'GC(opp)', 'xG(opp)', 'xGA(opp)', 'Últimos 5(opp)', 'Máximo Goleador del Equipo(opp)'
        ],
        'goals': [
            'Día', 'Sedes',
            # Offensive stats
            ...
            'GF(tm)', 'xG(tm)', 'Ass(tm)',
            # Defensive stats
            'TklG(tm)', 'Int(tm)', 'Err(tm)', 'GC(tm)', 'xGA(tm)',
            # Form
            'Últimos 5(tm)', 'PG(tm)', 'PE(tm)', 'PP(tm)',
            # Opponent stats
            'TPint(opp)', 'TklG(opp)', 'Int(opp)', 'Err(opp)',
            'GC(opp)', 'xGA(opp)', 'Últimos 5(opp)'
        ],
        'corners': [
            'Día', 'Sedes',
            # Team stats
            'TPint(tm)', 'PrgC(tm)', '% de TT(tm)',
            'Corners_Favor(tm)', 'Corners_Contra(tm)',
            # Opponent stats
            'TPint(opp)', 'TklG(opp)', 'Int(opp)',
            'Corners_Favor(opp)', 'Corners_Contra(opp)'
        ],
        'yellow_cards': [
            'Día', 'Sedes',
            # Team disciplinary stats
            'Err(tm)', 'TklG(tm)', 'Int(tm)',
            'Amarillas(tm)', 'Rojas(tm)',
            # Opponent disciplinary stats
            'Err(opp)', 'TklG(opp)', 'Int(opp)',
            'Amarillas(opp)', 'Rojas(opp)'
        ]
    }

    # Define target variables for each prediction type
    targets = {
        'match_outcome': ['Resultado'],
        'goals': ['GF', 'GC'],
        'corners': ['Corners_Favor(tm)', 'Corners_Contra(tm)'],
        'yellow_cards': ['Amarillas(tm)', 'Amarillas(opp)']
    }

    # Create feature matrices and target vectors for each prediction type
    X_sets = {}
    y_sets = {}

    for pred_type, features in feature_sets.items():
        # Select features
        X = df[features]
        
        # Select corresponding targets
        y = df[targets[pred_type]]
        
        # Adjust match outcome labels (0-based indexing)
    ...
    scaler = StandardScaler()
        ...
        # Save scaler for this prediction type
    pathlib.Path("models").mkdir(exist_ok=True)
        with open(f"models/scaler_{pred_type}.pkl", "wb") as f_out:
        pickle.dump(scaler, f_out)

        logger.info(f"Processed features for {pred_type} prediction:")
...
def pipeline_entrenamiento(jornada_actual: int, target: Optional[str] = None):
    ...
    """
    try:
        # Set up MLflow to use local file system
    ...
    df = actualizar_dataset(file_path, jornada_actual)
        ...
        quality_metrics = quality_checker.check_quality(df)
        
        if quality_metrics.overall_score < 0.8:
            logger.error(f"Data quality issues found: {quality_metrics.issues}")
            logger.error(f"Overall quality score: {quality_metrics.overall_score}")
            raise ValueError("Data quality score below threshold")
            
        if not quality_checker.validate_minimum_data(df):
        ...
        with mlflow.start_run(run_name=f"training_jornada_{jornada_actual}"):
            ...
            # Continue with model training based on target
            if target:
                logger.info(f"Training model for target: {target}")
                train_specific_model(df, target)
            else:
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Yellow cards are already calculated in process_match_dataframe
    
    # Calculate rolling averages for disciplinary stats
    for window in [3, 5]:
        df[f'Yellow_Cards_Tm_Last{window}'] = df.groupby('Anfitrion')['Yellow_Cards(tm)'].transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=1).mean()
        )
    
    # Calculate rolling averages for disciplinary stats
    for window in [3, 5]:
        for stat in ['Yellow_Cards', 'Fouls_Committed', 'Fouls_Received', 'Foul_Ratio']:
    ...
    if 'Tkl' in df.columns and 'TklG' in df.columns:
        ...
        logger.warning("Tackles data not available, skipping defensive features calculation")
        # Add placeholder columns with default values
        df['Tackles(tm)'] = 0
        df['Successful_Tackles(tm)'] = 0
        df['Tackle_Success_Rate(tm)'] = 0
        for window in [3, 5]:
            for stat in ['Tackles', 'Successful_Tackles', 'Tackle_Success_Rate']:
                df[f'{stat}_Tm_Last{window}'] = 0
    ...
    df['Shot_Conversion_Rate'] = df['GF'] / df['Shots(tm)'].replace(0, 1)
    df['SoT_Conversion_Rate'] = df['GF'] / df['SoT(tm)'].replace(0, 1)
    df['SoT_Accuracy'] = df['SoT(tm)'] / df['Shots(tm)'].replace(0, 1)
